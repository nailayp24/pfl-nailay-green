...
const generateOrderData = () => {
  ...
  const statuses = ["Paid", "Refunded", "Pending"]; // Disesuaikan dengan status di BengkelGo
  
  return Array.from({ length: 15 }, (_, i) => {
    const date = new Date(2026, 4, Math.floor(Math.random() * 4) + 1);
    return {
      orderId: `BG-00${540 + i}`,
      customerName: customers[i % customers.length],
      email: customers[i % customers.length].toLowerCase().replace(" ", "") + "@gmail.com",
      vehicle: i % 2 === 0 ? "Vario v22" : "NMAX Turbo",
      status: statuses[i % 3],
      cost: Math.floor(Math.random() * 500000) + 50000,
      orderDate: date.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })
    };
  });
};
...
export default function ServiceList() {
  ...
  const [formData, setFormData] = useState({ customerName: '', status: 'Paid', cost: '', orderDate: '', vehicle: '' });

  const handleAddOrder = (e) => {
    e.preventDefault();
    const newOrder = {
      orderId: `BG-00${540 + orders.length + 1}`,
      email: formData.customerName.toLowerCase().replace(" ", "") + "@gmail.com",
      ...formData,
      cost: parseInt(formData.cost),
      orderDate: new Date(formData.orderDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })
    };
    setOrders([newOrder, ...orders]);
    setShowModal(false);
    setFormData({ customerName: '', status: 'Paid', cost: '', orderDate: '', vehicle: '' });
  };
  ...
  return (
    <div className="p-2 space-y-6">
      ...
      <div className="bg-[#1A1C23] rounded-[30px] p-8 shadow-xl border border-gray-800">
        <div className="flex justify-between items-start mb-10">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Daftar perbaikan kendaraan aktif di BengkelGo.</p>
          </div>
          <button 
          ...
          <table className="w-full text-left">
            ...
            <tbody className="text-sm">
              {orders.map((item, index) => (
                <tr key={index} className="border-b border-gray-800/50 hover:bg-[#2D3748]/20 transition-all group">
                  <td className="py-5">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-orange-400 to-yellow-200 border-2 border-[#FF8A00]"></div>
                      <div>
                        <p className="font-bold text-white leading-none">{item.customerName}</p>
                        <p className="text-[11px] text-gray-500 mt-1">{item.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-5 text-gray-400 font-mono">{item.orderId}</td>
                  <td className="py-5 text-gray-300">{item.vehicle}</td>
      ...
      {/* Modal - Disesuaikan dengan tema Dark */}
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#1A1C23] border border-gray-800 rounded-[30px] shadow-2xl max-w-md w-full p-8">
            ...
            <form onSubmit={handleAddOrder} className="space-y-4">
              ...
              />
              <input 
                type="text" placeholder="Vehicle (e.g. PCX 160)" required
                className="w-full bg-[#2D3748] border border-gray-700 rounded-xl p-3 text-white focus:border-[#FF8A00] outline-none"
